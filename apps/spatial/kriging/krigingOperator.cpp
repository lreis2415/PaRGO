...
std::multimap<double, SamplePoint> KrigingOperator::searchNbrSamples(const int subMinRow, int cellRow, int cellCol, double cellX, double cellY) {
    ...
    while (isSearch) {
        double searchRange = static_cast<double>(searchRad - 1) * _blockSize + minDistToBound; // actual search range
...
bool KrigingOperator::Operator(const CellCoord& coord, bool operFlag) {
    ...
    if (_pMaskLayer) {
        ...
        if (maskRow >= _pMaskLayer->cellSpace()->nRows()) {
            if (iCol == 0) {
                ...
                double scale = static_cast<double>(_pMaskLayer->metaData()->_glbDims.nRows()) / _pIDWLayer->metaData()->_glbDims.nRows();
    ...
    vector<double> distanceCovariogramVector = CalculateCovariogramVector(dataPointCandidate, cellX, cellY, Spherical, GetEstimatedNugget(), GetEstimatedSill(), GetEstimatedRange(), true);

    vector<vector<double>> distanceCovariogramMatrix = CalculateCovariogramMatrix(dataPointCandidate, Spherical, GetEstimatedNugget(), GetEstimatedSill(), GetEstimatedRange(), true);

...
bool FCMOperator::Operator(const CellCoord &coord, bool operFlag)
{
	...
	if( iRow==_xSize-2 && iCol==_ySize-2 ){
		endtime = MPI_Wtime();
		cout<<"myrank "<<_rank<<" time "<<endtime-starttime<<endl;
	}
	//return true;
	...
	if( ( _iterNum != 0 ) && (fabs((*(vInputL[0]))[iRow][iCol]-_noData) <= Eps || fabs((*(vInputL[0]))[iRow][iCol]+9999) <= Eps) && !((iRow==_xSize-2) && (iCol==_ySize-2)) ){
	...
	if( (iRow==1) && (iCol==1) ){
		if( (_iterNum==0) ){
			...
			if( _rank == 0 ){
				...
				//do{
				//	centerFlag = true;
				//	createRandomIdx(clusterNum, _xSize*_ySize);	//产生的聚类中心直接放入centerIndex
				//	//createRandomIdx(clusterNum, nRows*nCols, centerIndex);	//测试用
				//	//判断聚类中心是否为空值，若为空值，重新产生
				//	for( int i=0; i<clusterNum; ++i ){
				//		int tmpRow = centerIndex[i] / nCols;
				//		int tmpCol = centerIndex[i] % nCols;	//从产生的随机数映射回对应的行列号
				//		//获取聚类中心属性值
				//		for( int j=0; j<imageNum; ++j ){
				//			centerVal[i*imageNum+j] = (*(vInputL[j]))[tmpRow][tmpCol];
				//			if( j==0 ){
				//				if( fabs(centerVal[i*imageNum+j] - _noData)<Eps ){
				//					//cerr<<"tmpRow = "<<tmpRow<<" tmpCol = "<<tmpCol<<endl;
				//					centerFlag = false;
				//					break;
				//				}
				//			}
	...
	if( fabs(pInputVal[0]+9999) <= Eps ||  fabs(pInputVal[0]-_noData) <= Eps){
		if( _iterNum == 0 ){	//空值仅第一次处理即可
			nodataNums++;
			for(int i=0; i<clusterNum; i++)
			{
				dist[i][iRow][iCol] = _noData;
				degree[i][iRow][iCol] = _noData;
			}
			fcmL[iRow][iCol] = _noData;
		}
	}else{
	...
	if( (iRow==_xSize-2) && (iCol==_ySize-2) ){
		...
		if( (fabs(totval-oldtval)<=tolerance) || (_iterNum>=maxIteration) ){
			//输出聚类中心	//这里的输出与后面冗余
			if( _rank==0 ){
				cout<<"final cluster center value is:"<<endl;
				for(int i=0;i<clusterNum;i++){
					cout<<i<<": ";
					for(int j=0;j<imageNum;j++){
						cout<<centerVal[i*imageNum+j]<<", ";
				...
				for(int j=1; j<_ySize-1; j++){
					if( (*(vInputL)[0])[i][j] != _noData ){
						...
						for( int p=0; p<clusterNum; p++ ){
							...
							if((degree[p][i][j]-_noData)>Eps){	//这判断应该是冗余的
			...
			for(int p=0; p<clusterNum; p++){
				...
				//计算分子分母,求新聚类中心
				int valCount = 0;
				for(int i=1; i<_xSize-1; i++){	//一定要注意这里只计算有效空间
					for(int j=1; j<_ySize-1; j++){
						if(fabs((*vInputL[0])[i][j]-_noData)>Eps && fabs((*vInputL[0])[i][j]+9999)>Eps ){
			...
			Termination = 0;
			//if( _rank==0 ){
			//	//输出本次迭代新产生的聚类中心,熵，分类系数
			//	cout<<"next cluster center is"<<endl;
			//	for(int i=0;i<clusterNum;i++){
			//		cout<<i<<"/"<<clusterNum<<" : ";
			//		for(int j=0;j<imageNum;j++){
			//			cout<<centerVal[i*imageNum+j]<<", ";
	...
	//vector<CellSpace<double>* > tmpV1, tmpV2;
	//vInputL.swap(tmpV1);
	//vDegL.swap(tmpV2);
	delete pInputVal;

...
bool SlopeOperator::Operator(const CellCoord &coord, bool operFlag)
{
	...
	{
			//三阶反距离权差分（ Third-order finite difference weighted by reciprocal of distance）
		double dx = (d[8] + sqrt(2.0)*d[5] + d[2] - d[6] - sqrt(2.0)*d[3] - d[0]) / ((4.0 + sqrt(8.0))*cellSize);
		double dy = (d[2] + sqrt(2.0)*d[1] + d[0] - d[6] - sqrt(2.0)*d[7] - d[8]) / ((4.0 + sqrt(8.0))*cellSize);

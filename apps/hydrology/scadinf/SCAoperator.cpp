...
double prop(double a, int k, double dx1, double dy1) {
    ...
    double aref[10] = {
        -atan2(dy1, dx1), 0., -aref[0], static_cast<double>(0.5 * PI),PI - aref[2], static_cast<double>(PI),
        PI + aref[2], static_cast<double>(1.5 * PI), 2. * PI - aref[2], static_cast<double>(2. * PI)
    };
    double p = 0.;
    if (k <= 0)k = k + 8;
    if (k == 1 && a > PI)a = static_cast<float>(a - 2.0 * PI);
